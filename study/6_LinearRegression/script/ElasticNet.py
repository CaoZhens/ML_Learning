...
if __name__ == "__main__":
    ...
    for i, d in enumerate(d_pool):
        elasticNet.set_params(poly__degree = d)
        elasticNet.fit(x, y.ravel())
        lin = elasticNet.get_params('linear')['linear']
        ...
        x_hat.shape = -1, 1
        y_hat = elasticNet.predict(x_hat)
        s = elasticNet.score(x, y)
    ...
    plt.grid(True)
    plt.title('ElasticNet', fontsize=14)
